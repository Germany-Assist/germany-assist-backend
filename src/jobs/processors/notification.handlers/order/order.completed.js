...
// triggered only by worker to complete the order after the escrow
...
async function handleOrderCompleted({ orderId }) {
  ...
  const providerMessage = `Successfully completed order ${hashedOrderId} of service "${order.Service.title}". Your money will be available to checkout shortly`;

  const userMessage = `The service provider ${order.ServiceProvider.name} completed your order ${hashedOrderId} for service "${order.Service.title}"`;
...
export default handleOrderCompleted;
